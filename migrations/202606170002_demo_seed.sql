...
VALUES
    ...
    ('group-demo-support', 'Demo Support', 'Abstimmung fuer Demo, Readback und offene Gates', 'user-david', '2026-06-17T09:05:00Z', '2026-06-17T09:05:00Z')
ON CONFLICT (id) DO NOTHING;
...
VALUES
    ...
    ('thread-matrix-link', 'group-team-1', 'Matrix-Verknuepfung klaeren', 'question', 'user-samira', 'open', '2026-06-17T09:30:00Z', '2026-06-17T09:30:00Z'),
...
VALUES
    ('msg-1', 'thread-architecture', '!team1:matrix.local', '$event1', 'user-david', 'PostgreSQL bleibt Modul-Wahrheit, Matrix bleibt Chat-Layer.', 'high', 0.91, 'cached', '2026-06-17T09:25:00Z'),
    ('msg-2', 'thread-matrix-link', '!team1:matrix.local', '$event2', 'user-samira', 'Fuer Submit zuerst vorhandene oder Dummy-Raeume verlinken.', 'normal', 0.66, 'cached', '2026-06-17T09:36:00Z')
ON CONFLICT (id) DO NOTHING;
...
VALUES
    ('wiki-db-plan', 'group-team-1', 'DB-Plan Kommunikation', 'PostgreSQL speichert Gruppen, Wiki, Feed, Graph und Agent-Feedback.', ARRAY['db', 'matrix', 'agent'], 'user-david', 'published', '2026-06-17T10:00:00Z', '2026-06-17T10:00:00Z'),
    ('wiki-matrix-postgres', 'group-team-1', 'Matrix und PostgreSQL', 'Matrix ist Chat-Layer. PostgreSQL bleibt Modul-Wahrheit.', ARRAY['matrix', 'postgresql'], 'user-samira', 'published', '2026-06-17T10:05:00Z', '2026-06-17T10:05:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO feed_items (id, group_id, source_type, source_id, title, summary, priority, created_at)
VALUES
    ('feed-1', 'group-team-1', 'agent_feed_item', 'agent-1', 'Wichtige DB-Entscheidung', 'PostgreSQL ist Modul-Wahrheit, Matrix bleibt Chat-Layer.', 'high', '2026-06-17T10:12:00Z'),
    ('feed-2', 'group-team-1', 'wiki_article', 'wiki-matrix-postgres', 'Matrix-Linking dokumentiert', 'User- und Raum-Links bleiben getrennt vom Profil.', 'normal', '2026-06-17T10:13:00Z'),
...
VALUES
    ...
    ('node-group-team-1', 'group', 'Team 1 Kommunikation', 'Gruppe fuer Kommunikation und Wissen', 'group', 'group-team-1'),
    ('node-thread-architecture', 'decision', 'DB-Entscheidung', 'PostgreSQL bleibt Modul-Wahrheit', 'thread', 'thread-architecture'),
    ('node-wiki-matrix-postgres', 'wiki_article', 'Matrix und PostgreSQL', 'Trennung Chat-Layer und Modul-Wahrheit', 'wiki_article', 'wiki-matrix-postgres'),
    ('node-agent-1', 'agent_item', 'Naechste Schritte DB-Integration', 'Mock-Agent erzeugt Taskliste', 'agent_feed_item', 'agent-1')
ON CONFLICT (id) DO NOTHING;
...
VALUES
    ('agent-1', 'group-team-1', 'task_list', 'Naechste Schritte DB-Integration', '{"tasks":["PostgreSQL-Service vorbereiten","Schema initialisieren","Matrix-Room-Link testen"]}', 'thread', 'thread-architecture', 'high', 0.84, 'new', '2026-06-17T10:20:00Z'),
    ('agent-2', 'group-team-1', 'message_priority', 'Prioritaet: Matrix-Event-Links', '{"priority":"high","reason":"Feed, Graph und Agent brauchen stabile Event-Referenzen."}', 'message_cache', 'msg-1', 'high', 0.78, 'new', '2026-06-17T10:21:00Z'),
    ('agent-3', 'group-demo-support', 'summary', 'Readback-Zusammenfassung', '{"summary":"Health, Useradapter, OpenAPI und Demo-Daten lokal pruefen; Docker/Synapse bleiben externe Gates."}', 'thread', 'thread-demo-flow', 'normal', 0.72, 'seen', '2026-06-17T10:22:00Z')
ON CONFLICT (id) DO NOTHING;

INSERT INTO agent_feedback (id, agent_feed_item_id, user_id, value, reason, created_at)
VALUES
    ('feedback-1', 'agent-1', 'user-david', 1, 'Hilft fuer die naechste Umsetzung.', '2026-06-17T10:25:00Z'),
